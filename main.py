...
ВЕРНИТЕ ТОЛЬКО JSON БЕЗ ДОПОЛНИТЕЛЬНОГО ТЕКСТА."""
        ...
        # ДЕТАЛЬНОЕ ЛОГИРОВАНИЕ КАК В ДИАГНОСТИКЕ
...
async def analyze_grouping_diagnostic(files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        # Упрощенный промпт для точного анализа товаров - ИДЕНТИЧЕН ОСНОВНОЙ ВЕРСИИ
...
КРИТИЧЕСКИ ВАЖНО - НЕ ПУТАЙ РАЗНЫЕ КАТЕГОРИИ:
- Детские электромобили ≠ Детские коляски
- Черный цвет ≠ Белый цвет (даже одной модели)
- Mercedes G-Class ≠ Ford Raptor (разные модели)
- Качели-шезлонг ≠ Электромобили (совершенно разные товары)

ПРИМЕРЫ ПРАВИЛЬНОЙ ГРУППИРОВКИ:
✅ Mercedes G-Class черный (фото 1,2,5) - одна группа
✅ Mercedes G-Class белый (фото 3,4,6) - отдельная группа  
✅ Детские качели (фото 15,16) - отдельная группа
❌ НЕ объединяй коляски с машинками!
❌ НЕ объединяй разные цвета одной модели!

Анализируй изображения с максимальной точностью. Группируй только абсолютно идентичные товары. При малейших сомнениях создавай отдельные группы.""",
                messages=[
                    {
                        "role": "user",
                        "content": [
